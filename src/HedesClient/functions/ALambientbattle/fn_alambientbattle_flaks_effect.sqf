...
//if (!isNil {player getVariable "flaks_on"}) exitwith {};player setVariable ["flaks_on",true,true];
...
while {al_aaa} do 
{
	...
	if (_zz<500) then {_li_aaa setLightFlareSize 10+random 100;_li_aaa setLightIntensity 500+random 500;};
	if (_zz>800) then 
	{
		[_li_aaa] spawn 
		{
