			...
			// Iterate through default or synchronized triggers.
			{
				...
				while {_numveh > count(_vehtracker select {alive _x} select {[_trigger, _x] call BIS_fnc_inTrigger }) && _vehi < _maxtryveh} do
				{
					...
					_spawnpos = if(_isfirstspawn == 1) then {
						[_trigger call BIS_fnc_randomPosTrigger, 5, 10] call BIS_fnc_findSafePos
					} else {
						[_trigger, false, 5] call FUNCMAIN(FindHiddenRanPosInMarker)
					};
					...
					if !(_pos isEqualTo [0,0]) then {
						_pos = [_pos, 5, 10] call BIS_fnc_findSafePos;
